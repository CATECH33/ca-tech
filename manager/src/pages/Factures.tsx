...
import {
  ...
  useEnvoyerFacture, useFacturePayments, useCreateStripeCheckout,
...
function StripeModal({ facture, onSave, onClose }: {
  ...
  const handleGenerate = async () => {
    setGenError(null)
    try {
      const url = await createCheckout.mutateAsync(facture)
      setLink(url)
    } catch (err) {
  ...
  return (
    <Modal
      ...
      description={`${facture.numero} · ${formatCurrency(remaining)} restant`}
      footer={
        <>
    ...
    >
      <div className="space-y-4">
        ...
        <div className="p-4 bg-brand-50 border border-brand-100 rounded-xl">
          <p className="text-xs font-semibold text-brand-800 mb-2">Génération automatique</p>
          <p className="text-xs text-brand-700 mb-3">
            Créez une session de paiement Stripe en un clic pour{' '}
          ...
          >
            ...
            {createCheckout.isPending ? 'Génération…' : 'Générer le lien Stripe'}
          </Button>
          {genError && (
            <p className="text-xs text-red-600 mt-2 flex items-center gap-1">
              <AlertCircle className="h-3 w-3" />{genError}
            </p>
          )}
          {createCheckout.isSuccess && (
            <p className="text-xs text-emerald-600 mt-2 flex items-center gap-1">
              <Check className="h-3 w-3" />Lien généré — valable 30 jours
            </p>
          )}
        </div>

        {/* Link display / manual fallback */}
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">
            Lien de paiement
            <span className="ml-1 font-normal text-gray-400">(ou saisie manuelle)</span>
          </label>
